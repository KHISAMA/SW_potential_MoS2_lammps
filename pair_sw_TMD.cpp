...
PairSW_TMD::PairSW_TMD(LAMMPS *lmp) : Pair(lmp)
{
  single_enable = 0;
  restartinfo = 0;
  one_coeff = 1;
...
void PairSW_TMD::compute(int eflag, int vflag)
{
  ...
  //double bondss = 18.232900000; //14.592400000;
  ...
  for (ii = 0; ii < inum; ii++) {
	...
	//if (rsq2 > bondss) continue;
        ...
        if (rsq3 > params[ijkparam].cutrjk) continue; //r23 implementation
...
void PairSW_TMD::settings(int narg, char **arg)
{
...
void PairSW_TMD::coeff(int narg, char **arg)
{
  int i,j,n;
...
double PairSW_TMD::init_one(int i, int j)
{
...
void PairSW_TMD::read_file(char *file)
{
  ...
  while (1) {
    ...
    params[nparams].rjk = atof(words[14]); //r23 implementation
...
void PairSW_TMD::setup_params()
{
  ...
  for (m = 0; m < nparams; m++) {
    ...
    params[m].cutrjk= params[m].rjk * params[m].rjk; //r23 implementation
...
void PairSW_TMD::twobody(Param *param, double rsq, double &fforce,
...
void PairSW_TMD::threebody(Param *paramij, Param *paramik, Param *paramijk,
